...
while(not flag):
    ...
    for (top, right, bottom, left), face_encoding in zip(
        ...
        cv2.putText(frame, gender, (left+5, bottom+h/10+22 ), cv2.FONT_HERSHEY_TRIPLEX, 1.0,
                    (100, 100, 0), 1)
        cv2.putText(frame, emotion, (left+5, bottom+h/10+44), cv2.FONT_HERSHEY_TRIPLEX, 1.0,
                    (0, 100, 100), 1)
        cv2.putText(frame, name, (left+5 , bottom+h/10+66), cv2.FONT_HERSHEY_TRIPLEX, 1.0,
                    (0, 0, 100), 1)

    #when False, continue
    flag= ((operatorNum==1) and (lastfaceNum==faceNum))
